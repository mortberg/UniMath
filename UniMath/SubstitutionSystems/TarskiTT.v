...
Variable (Code Ty : hSet).
Definition sort : hSet := (Code + Ty)%set.

Definition code (c : Code) : sort := inl c.
Definition ty (t : Ty) : sort := inr t.

(* I think we need these operations *)
Variable (Pi : (hset_fun_space Ty Ty) → Ty). (* The sort of types has to be closed under Pi *)
Variable (UT : Ty). (* Universe a la Tarski *)
Variable (El : (Ty → Ty) → sort). (* Decoding function turning codes for types in actual types *)
...
Definition I : hSet :=
  ( (Ty × (hset_fun_space Ty Ty)) (* lam *)
  + (Ty × (hset_fun_space Ty Ty)) (* app *) (* This should be a dependent sum instead *)
  )%set.

Definition TT_Sig : MultiSortedSig sort.
Proof.
use mkMultiSortedSig.
- exact I.
- intros H.
  induction H as [AB|AB]; induction AB as [A B].
  + exact (((cons (ty A) [],,ty (Pi B)) :: []),,ty (Pi B)).
  + exact ((([],,ty (Pi B)) :: ([],,ty A) :: nil),,ty (B A)).
Defined.
